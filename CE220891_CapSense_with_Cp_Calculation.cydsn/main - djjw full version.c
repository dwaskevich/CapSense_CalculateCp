...
#define LOW_CP_LIMIT_FEMTOFARADS    (7000u)
...
{	        
    ...
    */
    ...
    UART_UartPutString(printBuf);
    sprintf(printBuf, "CapSense_CSD_IDACA_REG = 0x%08lx\r\n", CapSense_CSD_IDACA_REG);
    UART_UartPutString(printBuf);
    ...
    for(;;)
    {
        ...
        switch(currentState)
        {
            ...
            case PROCESS_DATA:
                ...
                /* test to see if there are any open sensors (less than LOW_CP_LIMIT_FEMTOFARADS) */
                LED2_Write(1);
                for(uint8 i = 0; i < CapSense_TOTAL_CSD_SENSORS; i++)
            	{
            		if(RegisterMap.SensorCp_Calculated[i] < LOW_CP_LIMIT_FEMTOFARADS)
                    {
                        LED2_Write(0);
                    }
                }
